...
- **F8.9 — Segurança completa ✅ (2026-09-22)**: CORS **restrito** por env (`FRONT_URL` + `CORS_ORIGINS`), **Helmet** (CORP `cross-origin` p/ os `<img>` de logos/galeria), **rate limit** (`@nestjs/throttler` global 100/min + `@Throttle` 10/min em `/auth/*` e `POST /l/:slug/pedidos`, `trust proxy` p/ Render), **validação no boot** do `APP_JWT_SECRET` (< 32 bytes → falha em produção), auditoria de logs/raw/validação (sem mudanças necessárias — front escapa tudo com `{{ }}`). **Pendente (requer dono, pós-deploy)**: preencher `FRONT_URL`/`CORS_ORIGINS` com o domínio real do Vercel. Ver item em `debitos_tecnicos.md`.
