...
## 0. Andamento (atualizado em 2026-09-22)
...
- **Etapa 11 — Separação de perfis ✅ (2026-09-22)**: ver seção própria abaixo. Decisão do dono: **bloqueio total por perfil principal** (admin-sistema > dono > cliente).
...
- **Etapa 7 — Importação de cardápio com IA (⏸️ deferida)**: ver item F8.7 em `debitos_tecnicos.md` (plano + decisões em aberto + como retomar).
- **F8.11 — Contrato de assinatura offline + R2**: ver item em `debitos_tecnicos.md` (desenhado; nada implementado).
...
- **F8.7 — Cardápio com IA (texto | planilha | foto | voz) ⏸️ deferida**: adiada por decisão do dono (2026-09-22) para planejar melhor — **plano completo + 3 decisões pendentes + mensagem de retomada em `debitos_tecnicos.md`** (item F8.7). Não começar antes de F8.8/F8.9/F8.10.
- **F8.8 — Planos de assinatura ✅ (2026-09-22)**: `Lanchonete.plano String @default("trial")` (`trial|pago`) + `planoExpira DateTime?` (migração `m15_plano_assinatura`; **backfill**: lojas existentes ganharam 30d a partir de hoje). Loja nova nasce `trial` + 30d (no `create()` do AdminService e no seed `seed:teste`). **Bloqueio real** em `criaPedido` (`pedidos.service.ts`): trial expirado → 400 "O período de teste desta lanchonete expirou…" (cardápio público continua visível). Painel plataforma: coluna **Plano** na lista (badge `Trial · até DD/MM/AAAA`, `Trial expirado` vermelho, `Pago` verde) + botão na consulta "Marcar como pago/teste" via `PATCH /admin-sistema/lanchonetes/:id/plano` (com log de ação "Plano de assinatura alterado"); **decisão do dono**: `pago` → `planoExpira` null (sem expiração até o gateway; R$ 149,90/mês fica para a cobrança real) e voltar a `trial` reinicia +30d. Dono: banner no `admin-config` (teste com data limite, **expirado em vermelho**, pago verde). Back `npm test` = **72** (+5 `alterarPlano`). **Pendente (requer dono)**: landing page de assinatura + gateway (Mercado Pago/PagSeguro) para cobrança real e renovação automática.
- **F8.9 — Segurança completa ✅ (2026-09-22)**: CORS **restrito** por env (`FRONT_URL` + `CORS_ORIGINS`), **Helmet** (CORP `cross-origin` p/ os `<img>` de logos/galeria), **rate limit** (`@nestjs/throttler` global 100/min + `@Throttle` 10/min em `/auth/*` e `POST /l/:slug/pedidos`, `trust proxy` p/ Render), **validação no boot** do `APP_JWT_SECRET` (< 32 bytes → falha em produção). **Pós-auditoria (mesmo dia)**: `requerSuper` em criar/editar/remover admins e em `PATCH /admin-sistema/plataforma`; consulta de loja (`GET /admin-sistema/lanchonetes/:id`) mascara `chavePix` para admin humano; `THROTTLE_DISABLED=true` **bloqueado** em produção; `@Throttle` 5/min em `/auth/verificar-email` e 30/min nos GET públicos de lojas; CORS do WebSocket espelhado (`FRONT_URL`+`CORS_ORIGINS`); `algorithms:['EdDSA']` explícito no jwtVerify legado da Neon; **removido o default automático** do e-mail público do raiz (home usa fallback até configurar); aba Configurações do painel só p/ raiz (`eSuper()` no front). **Pendente (requer dono, pós-deploy)**: preencher `FRONT_URL`/`CORS_ORIGINS` com o domínio real do Vercel. Ver item em `debitos_tecnicos.md`.
- **F8.10 — LGPD + Termo de Uso ✅ (2026-09-22)**: termo único combinado (Termos de Uso + Política de Privacidade) na página pública `/termos` (rota no front antes do `:slug`; conteúdo estático pt-BR + e-mail de contato via `GET /plataforma/email-lojista` como contato LGPD). **Consentimento gravado no banco**: tabela `termos_aceite` (`usuarioId` uuid do Neon Auth + `versao` + `aceitoEm`, PK composta `[usuarioId, versao]`, migração `m16_termo_aceite`); `POST /me/consentimento` (`AuthGuard`) valida `versao === TERMO_VERSAO_ATUAL` ('1.0', const em `back/src/me/termo.ts`, espelhada no front em `services/termos.ts`) → upsert **preserva a data do 1º aceite** (`update: {}`); `GET /me` expõe `termoAceite` (último aceite). **Fluxo no front** (`auth.component`): checkbox "Li e aceito os Termos de Uso e a Política de Privacidade" **obrigatório no cadastro** (bloqueia o submit; após o sign-up o aceite é gravado automaticamente); usuário existente/Google sem aceite da versão atual cai na tela **"Aceite os Termos de Uso"** (botão "Aceitar e continuar" + "Sair desta conta") antes de seguir. Back `npm test` = **78** (+6 `me.service.spec.ts`).
- **F8.11 — Contrato de assinatura offline (desenhada, nada feito — ver item em `debitos_tecnicos.md`)**: adesão **presencial sem landing**: o dono leva um **contrato físico** à lanchonete (orientações de pagamento: **chave PIX dele** + **e-mail/whatsapp** para o lojista enviar o comprovante). Assinado, o contrato é **escaneado e fica disponível para a lanchonete baixar** (painel do dono) e conferir; super-admin também gerencia. **Armazenamento = R2** (bucket já previsto no `R2Service` adormecido; **não** guardar no disco do Render que é efêmero, nem `BYTEA` na Neon) — PDF no bucket, só metadados no banco, **download autenticado** (LGPD). Landing + gateway (Mercado Pago/PagSeguro) da cobrança real/renew ficam para depois desta fase.

### Etapa 11 — Separação de perfis: Lanchonete × Cliente × Admin (✅ concluída 2026-09-22)

**Implementado**: `perfil.service.ts` (front) centraliza os perfis via `GET /me` (`dados` signal + computeds `ehDono`/`ehAdminSistema`/`perfilPrincipal()` = admin-sistema > dono > cliente; `carregar(forcar)` com cache de promise; `limpar()` chamado no `signOut` do `AuthService`). `perfil.guard.ts` substitui o `authGuard` (removido): `perfilGuard(['cliente'|'dono'|'admin-sistema'])` checa autenticação e **força reload do `/me`** a cada navegação protegida; quem não tem o perfil cai no **destino do perfil principal** (`/painel-admin`, `/admin` ou `/home`). Rotas: `/home` e `/minhas-pedidos` → `['cliente']`; `/admin` e `/admin/:slug/*` → `['dono']`; `/painel-admin` → `['admin-sistema']`; `/onboarding` → `onboardingGuard` (qualquer autenticado **sem** lanchonete; dono → `/admin`, admin-sistema → `/painel-admin`). Cardápio público `/{slug}` segue público e tem botão **dinâmico** conforme `perfilPrincipal()` (cliente → "Página principal"/home; dono → "Painel do dono"/admin; admin → "Painel da plataforma"). Back não mudou (defesa em profundidade já existia: dono-check 404 no `/admin/*`, `AdminSistemaGuard`). Sem migração. Build do front OK (só NG8102 pré-existentes).
...
4. **Multi-perfil (decidido com o dono em 2026-09-22)**: **bloqueio total por perfil principal** — dono não acessa `/home`/`/minhas-pedidos` (volta ao `/admin/:slug/config`); admin-sistema só vê `/painel-admin`; cliente em `/admin` cai em `/home` (que traz o convite "Quer vender pelo Peditto?"). O dono vê a própria loja pela página pública `/{slug}` (botão "Ver página"); dono que quiser comprar usa uma conta cliente separada.
...
### Etapa 7 — Importação de cardápio com IA (⏸️ deferida — ver item F8.7 em `debitos_tecnicos.md`)
